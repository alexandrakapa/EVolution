...
function EnergyConsPerDistrict(props) {
  ...
  useEffect( () => {
    if (didSubmit){
      ...
      fetch(`https://localhost:8765/evcharge/api/SessionsPerProvider/PerDistrict/${localStorage.id}/${region}/${startdate}/${enddate}`,{headers:{'Content-type':'application/json','x-access-token':tok}
      })
