const EnergyCostModel=require('../models/energy_cost.model');
...
check_day = (y, m, d) => {
  ...
  exports.getCostPerModel=(req, res) => {
  	...
  	//heck if model length is valid based on our database's corresponding attribute's type
	if ((req.params.model).length > 255 ) {
  		res.statusMessage = 'Bad Request';
  		res.status(400).send('Bad Request : Invalid car model');
  		return;
  	}
    ...
    if (req.query.format!='csv' && req.query.format!=undefined && req.query.format!='json'){
 	...
 	EnergyCostModel.getCostByModel(req, (err, data) => {
 		if (err) {
 			...
 			res.status(402).send('No Charging Sessions for Cars of this model for this period.');
 			return;
 		}
